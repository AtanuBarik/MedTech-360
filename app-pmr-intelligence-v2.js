...
function pmrReadinessFriction(rows){
  ...
  return '<div class="pmr-rf-map"><svg viewBox="0 0 '+W+' '+H+'">'+
    ...
    rows.map(r=>{const x=p+r.ready/100*(W-2*p),y=p+r.friction/100*(H-2*p);return '<g><circle cx="'+x+'" cy="'+y+'" r="10" fill="#e8005a" fill-opacity=".16" stroke="#e8005a" stroke-width="2"/><circle cx="'+x+'" cy="'+y+'" r="4" fill="#e8005a"/><text x="'+x+'" y="'+(y-14)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#4a2351">'+esc(r.persona)+'</text></g>'}).join('')+
