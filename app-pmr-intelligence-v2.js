...
function pmrReadinessFriction(rows){
  ...
  const W=680,H=380,p=55;
  return '<div class="pmr-rf-map"><svg viewBox="0 0 '+W+' '+H+'">'+
    '<rect x="'+p+'" y="'+p+'" width="'+((W-2*p)/2)+'" height="'+((H-2*p)/2)+'" fill="#fbf7fc"/><rect x="'+(W/2)+'" y="'+p+'" width="'+((W-2*p)/2)+'" height="'+((H-2*p)/2)+'" fill="#fff7fa"/><rect x="'+p+'" y="'+(H/2)+'" width="'+((W-2*p)/2)+'" height="'+((H-2*p)/2)+'" fill="#fafafa"/><rect x="'+(W/2)+'" y="'+(H/2)+'" width="'+((W-2*p)/2)+'" height="'+((H-2*p)/2)+'" fill="#f5fbf7"/>'+
    '<line x1="'+p+'" y1="'+(H/2)+'" x2="'+(W-p)+'" y2="'+(H/2)+'" stroke="#d8cfd9"/><line x1="'+(W/2)+'" y1="'+p+'" x2="'+(W/2)+'" y2="'+(H-p)+'" stroke="#d8cfd9"/>'+
    '<text x="'+(W*.25)+'" y="'+(p+18)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#7d7081">HIGH FRICTION / LOWER READINESS</text><text x="'+(W*.75)+'" y="'+(p+18)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#9c305a">HIGH FRICTION / READY</text>'+
    '<text x="'+(W*.25)+'" y="'+(H-p-10)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#7d7081">LOW FRICTION / LOWER READINESS</text><text x="'+(W*.75)+'" y="'+(H-p-10)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#27724c">LOW FRICTION / READY</text>'+
    rows.map(r=>{const x=p+r.ready/100*(W-2*p),y=H-p-r.friction/100*(H-2*p);return '<g><circle cx="'+x+'" cy="'+y+'" r="10" fill="#e8005a" fill-opacity=".16" stroke="#e8005a" stroke-width="2"/><circle cx="'+x+'" cy="'+y+'" r="4" fill="#e8005a"/><text x="'+x+'" y="'+(y-14)+'" text-anchor="middle" font-size="10" font-weight="800" fill="#4a2351">'+esc(r.persona)+'</text></g>'}).join('')+
    '<text x="'+(W/2)+'" y="'+(H-8)+'" text-anchor="middle" font-size="11" fill="#756b78">Readiness →</text><text x="17" y="'+(H/2)+'" transform="rotate(-90 17 '+(H/2)+')" text-anchor="middle" font-size="11" fill="#756b78">Implementation friction →</text></svg></div>';
}
...
function renderExpertPage(){
  ...
  $('pageContent').innerHTML=pmrPageHead("Expert & KOL Interview Analysis",'A cross-project synthesis of KOL interviews, expert transcripts and qualitative final reports for <b>'+esc(currentDomain)+'</b>, highlighting consensus, disagreement, emerging signals, evidence expectations, risks and implications.')+
    ...
    '<section class="section"><div class="grid-2"><div class="card"><div class="card-title"><h3>Theme heatmap</h3><small>Expert archetype × topic intensity</small></div>'+ci2Heatmap(experts.map(x=>x.persona),themes.map(x=>x.theme),(e,t)=>1+(pmrHash(currentDomain+e+t+pmrFilterText(s))%5),"Synthetic thematic coding. 5 = recurring / high-salience topic.")+'</div><div class="card"><div class="card-title"><h3>Readiness × friction</h3><small>Theme-level implementation outlook</small></div>'+pmrReadinessFriction(pmrThemeReadinessRows(themes,s))+'</div></div></section>'+
